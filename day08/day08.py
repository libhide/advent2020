...
value given in the argument. For example, acc +7 would increase the accumulator by
7. The accumulator starts at 0. After an acc instruction, the instruction immediately
...
almost any of the jmp instructions, the program will still eventually find another 
...
def execute_program(instructions: list) -> tuple:
    ...
    visited_indexes = []
    while exec_pointer < len(instructions):
        if exec_pointer in visited_indexes:
            return accumulator, True

        visited_indexes.append(exec_pointer)
        operation, arg = instructions[exec_pointer].split(" ")

        if operation == "acc":
            accumulator += int(arg)
        elif operation == "jmp":
            exec_pointer += int(arg)
            continue
        else:
            pass  # nop case
...
def solve_part1(instructions: list) -> int:
    acc, runs_infinitley = execute_program(instructions)
    return acc


def solve_part2(instructions: list) -> int:
